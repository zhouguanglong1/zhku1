...
<table class="table table-bordered table-hover definewidth m10" >
    ...
    <c:forEach items="${userList }" var="user">
	     <tr>
            ...
            <td><img src="/photo/${user.photoPath}" width=100 height=120/></td>
            <td>
            	<c:choose>
            ...
            <td>
            	<c:choose>
            		<c:when test="${user.roleStatus ==1 }">普通用户</c:when>
            ...
            <td>
            	<c:choose>
            		<c:when test="${user.status==1 }">
            			<button type="button" class="btn btn-warning" id="close" onclick="unable(${user.id });">禁用</button>
            		</c:when>
            		<c:otherwise>
            			<button type="button" class="btn btn-success" id="open" onclick="enable(${user.id });">启用</button>
            		</c:otherwise>
            	</c:choose>
                  
                  <button type="button" class="btn btn-danger" id="del" onclick="del(${user.id });">删除</button>
