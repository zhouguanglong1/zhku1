...
<header class="zhku-nav ">
    <nav class="navber w">
        ...
        <div class="n-search f_r">
            <div class="input f_l">
                <a href="javascript:void;" id="scbar_type" class="xg1" hidefocus="true">搜索</a>
                <form action="${pageContext.request.contextPath}/product/findByPname.action" method="get">
                <input type="text" name="srchtxt"  class=" xg1" placeholder="请输入搜索内容">
                <button type="submit" name="searchsubmit" class="pn pnc"><strong class="xi2"></strong></button>
                </form>
            </div>
            ...
            <div id="username">
            	<tr>
            		<td>
		            	...
		            	&nbsp;
		            	<a href="${pageContext.request.contextPath}/order/myOrder.action"><font color="white">我的订单</font></a>
	            	</td>
	            	&nbsp;
	            	<td>
...
<div class="container productList">
	...
	<div class="pagination">
		第  ${pageBean.page }/${pageBean.totalPage }页
			...
			<a href = "${pageContext.request.contextPath}/category/categoryList.action?page=1" >首页</a>
    	<c:choose>
    		<c:when test="${pageBean.page gt 1 }">
    			<a href = "${pageContext.request.contextPath}/category/categoryList.action?page=${pageBean.page -1} " >上一页</a> 
    		</c:when>
    		<c:otherwise>
    			&nbsp;
    		</c:otherwise>
    	...
    	<c:choose>
    		<c:when test="${pageBean.totalPage gt 1 && pageBean.page ne pageBean.totalPage}">
    			<a href = "${pageContext.request.contextPath}/category/categoryList.action?page=${pageBean.page +1} " >下一页</a>
    		</c:when>
    		<c:otherwise>
    			&nbsp;
    		</c:otherwise>
		...
		<a href = "${pageContext.request.contextPath}/category/categoryList.action?page=${pageBean.totalPage }">尾页</a> 
